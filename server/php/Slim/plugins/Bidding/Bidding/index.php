            ...
            if ($amount <= 0 && $project->project_status_id != \Constants\ProjectStatus::Draft) {
                if (!empty(PROJECT_IS_AUTO_APPROVE)) {                    
                    ...
                    $projectpushedNotification = array(
                            ...
                            '##PROJECT_URL##' => $_server_domain_url . '/project/' . $project->id.'/'.$project->slug);
...
$app->GET('/api/v1/projects/{projectId}', function ($request, $response, $args) {
    ...
    $project = Models\Project::with($enabledIncludes);
...
$app->GET('/api/v1/projectsinfo/{projectId}/{type}', function ($request, $response, $args) {
    global $authUser;
    $queryParams = $request->getQueryParams();
    $result = array();
    $project_status = array(
    ...
    if (!empty($project)) {
		...
		$result['data']['status'] = $project['project_status']['name'];
		$result['data']['days'] = $project['bid_duration'];
		$result['data']['bid_winner']['name'] = $project['bid_winner']['user']['username'];
		$result['data']['bid_winner']['id'] = $project['bid_winner']['user']['id'];
		$result['data']['bid_winner']['attachment'] = $project['bid_winner']['user']['attachment'];
		$result['data']['bid_winner']['last_login'] = date('Y-m-d H:i:s', strtotime($$project['bid_winner']['user']['updated_at']));
		$result['data']['project_user']['name'] = $project['user']['username'];
		$result['data']['project_user']['id'] = $project['user']['id'];
		$result['data']['project_user']['attachment'] = $project['user']['attachment'];
		$result['data']['project_user']['last_login'] = date('Y-m-d H:i:s', strtotime($project['user']['updated_at']));
		$result['data']['project_user']['is_loggedin'] = true;
		if ($request->getAttribute('type') == "Milestone") {
			$amount_released = Models\Milestone::select('amount')->where('project_id', $request->getAttribute('projectId'))->where('milestone_status_id', \Constants\MilestoneStatus::EscrowReleased)->get()->toArray();
			$amount_release = 0;
			if (!empty($amount_released)) {
				foreach ($amount_released as $amount_released) {
					$amount_release += $amount_released['amount'];
				}
			}
			$amount_requested = Models\Milestone::select('amount')->where('project_id', $request->getAttribute('projectId'))->where('milestone_status_id', '<',\Constants\MilestoneStatus::EscrowReleased)->get()->toArray();
			$amount_request = 0;
			if (!empty($amount_requested)) {
				foreach ($amount_requested as $amount_requested) {
					$amount_request += $amount_requested['amount'];
				}
			}
			$total_amount_released = Models\Milestone::select('amount')->where('project_id', $request->getAttribute('projectId'))->get()->toArray();
			$total_amount_release = 0;
			if (!empty($total_amount_released)) {
				foreach ($total_amount_released as $total_amount_released) {
					$total_amount_release += $total_amount_released['amount'];
				}
			}
			$result['data']['milestone']['amount'] = $total_amount_release;
			$result['data']['milestone']['amount_released'] = $amount_release;
			$result['data']['milestone']['amount_requested'] = $amount_request;
		} else {
			//invoice
			$amount_released = Models\ProjectBidInvoice::select('amount')->where('project_id', $request->getAttribute('projectId'))->where('is_paid',1)->get()->toArray();
			$amount_release = 0;
			if (!empty($amount_released)) {
				foreach ($amount_released as $amount_released) {
					$amount_release += $amount_released['amount'];
			...
			$amount_requested = Models\ProjectBidInvoice::select('amount')->where('project_id', $request->getAttribute('projectId'))->where('is_paid',0)->get()->toArray();
			$amount_request = 0;
			if (!empty($amount_requested)) {
				foreach ($amount_requested as $amount_requested) {
					$amount_request += $amount_requested['amount'];
			...
			$total_amount_released = Models\ProjectBidInvoice::select('amount')->where('project_id', $request->getAttribute('projectId'))->get()->toArray();
			$total_amount_release = 0;
			if (!empty($total_amount_released)) {
				foreach ($total_amount_released as $total_amount_released) {
					$total_amount_release += $total_amount_released['amount'];
			...
			$result['data']['inovice']['amount'] = $total_amount_release;
			$result['data']['inovice']['amount_released'] = $amount_release;
			$result['data']['inovice']['amount_requested'] = $amount_request;
		}
        return renderWithJson($result);
    } else {
        return renderWithJson($result, 'No record found', '', 1, 404);
                ...
                if (!empty($args['skills']) && $project->id) {
                    ...
                    if (!empty($args['skills'])) {
                        ...
                        foreach ($skills as $skill) {
                            ...
                            $skillsProjects = new Models\SkillsProjects;
                            $skillsProjects->skill_id = $newSkills->id ;
							$skillsProjects->created_at = date('Y-m-d h:i:s');
                            $skillsProjects->project_id = $project->id;
                            $skillsProjects->save();
                        }
                    }
                    ...
                    $employerDetails = getUserHiddenFields($project->user_id);
                        foreach ($followers as $follower) {
                            ...
                            $emailFindReplace = array(
                                ...
                                '##PROJECT_LINK##' => $_server_domain_url . '/project/' . $project->id . '/' . $project->slug 
                            );
...
$app->PUT('/api/v1/projects/{projectId}/update_status', function ($request, $response, $args) {
    ...
    try {
        ...
        return renderWithJson($result, 'Update status could not be updated. Please, try again.', $e->getMessage(), 1);
    }
...
$app->GET('/api/v1/me/projects/overview', function ($request, $response, $args) {
    global $authUser;
    $result = array();
    $queryParams = $request->getQueryParams();
	$queryParams['project_status_id'] = "11,15";
    $count = PAGE_LIMIT;
	$teamStrength = 0;
    if (!empty($queryParams['limit'])) {
        $count = $queryParams['limit'];
    ...
    $projects = Models\Project::with($enabledIncludes)->where('user_id', $authUser['id'])->Filter($queryParams)->paginate($count);
	$projectIds = Models\Project::select('id')->where('user_id', $authUser['id'])->Filter($queryParams)->get()->toArray();
	if ($projectIds) {
		$strength = Models\Bid::select('user_id')->whereIn('project_id', array_column($projectIds, 'id'))->get()->toArray();
		$teamStrength = count(array_unique(array_column($strength, 'user_id')));
	}
    if (!empty($projects)) {
        $projectsNew = $projects;
        $projectsNew = $projectsNew->toArray();
        $data = $projectsNew['data'];
        unset($projectsNew['data']);
		$data['teamStrength'] = $teamStrength;
        $result = array(
            'data' => $data,
            '_metadata' => $projectsNew
        );
...
$app->OPTIONS('/api/v1/project_categories', function ($request, $response, $args) {
	header("Access-Control-Allow-Origin: *");
    $queryParams = $request->getQueryParams();
    $results = array();
    global $authUser;
            ...
            if ($bid->save()) {
                if (!empty($args['is_freelancer_withdrawn'])) {                   
                    ...
                    $emailFindReplace = array(
                        ...
                        '##PROJECT_LINK##' => $_server_domain_url . '/projects/' . $bid->project->id . '/' . $bid->project->slug 
                    );
                ...
                if($old_amount != $newAmount || $old_description != $newDescription || $old_duration != $newDuration){
                    ...
                    $userDetails = getUserHiddenFields($bid->user_id);
                            $emailFindReplace = array(
                                ...
                                '##BUYER_USERNAME##' => ucfirst($userDetails->username) ,
                                '##PROJECT_LINK##' => $_server_domain_url . '/project/' . $bid->project->id . '/' . $bid->project->slug 
                            );
            ...
            if ($args['bid_status_id'] == \Constants\BidStatus::Won && in_array($bid['project']['project_status_id'], $wonProjectArray)) {
                if (!empty($args['new_winer_bid_id'])) {
                    ...
                    $emailFindReplace = array(
                        ...
                        '##PROJECT_LINK##' => $_server_domain_url . '/projects/' . $newBid->project->id . '/' . $newBid->project->slug 
                    );
...
$app->GET('/api/v1/projects/{user_id}/project_stats', function ($request, $response, $args) {
    ...
    try {
        ...
        $project['total_project_count'] = Models\Project::where('user_id', $userId)->where('project_status_id', '>', \Constants\ProjectStatus::OpenForBidding)->count();
        $results = array(
            'data' => $project
        );
        return renderWithJson($results);
