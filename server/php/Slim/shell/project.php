...
function update_onfido()
{
	...
	try {
		$users = Models\User::where('onfido_check_id', "!=", "")->where('onfido_status', "!=", "complete")->get();
		if (!empty($users)) {
			foreach ($users as $user) {
				$applicantId = $user->onfido_applicant_id;
				$checkId = $user->onfido_check_id;
				$headers = $result = array();
				$headers[] = 'Authorization: Token token='.$onfidoToken;
				$url = "https://api.onfido.com/v2/applicants/".$applicantId ."/checks/".$checkId;
				$applicantStatus = onfidoExecute($url, null, $headers);
				if($applicantStatus != "") {
					$userDetails = Models\User::find($user['id']);
					...
					$userDetails->update();
					if ($applicantStatusDetails['status'] == "complete") {
						$userInfo = getUserHiddenFields($userDetails->id);
						$emailFindReplace = array(
							'##USERNAME##' => $userInfo['username']
						);
						sendMail('onfidoNotification', $emailFindReplace, $userInfo['email']);
					}
				}
			}
		}
