...
$app->POST('/api/v1/adminStats', function ($request, $response, $args) use ($app)
{
	...
	if(!empty($lineChartResultSet)) {
		...
		foreach ($tempArray as $tmpArray) {
			$temp = array();
			$temp["xaxis"] = $tmpArray["xaxis"];
			$count = (int)($count + $tmpArray["total"]);
			$temp["total"] = $count;
			if ($year === true) {
				$newTempArr[date('d', strtotime($temp["xaxis"]))] = $temp;
			} else if ($month === true) {
				$newTempArr[$tmpArray['yearstat'].date('m', strtotime($temp['xaxis']))] = $temp;
			} else {
				$newTempArr[date('d', strtotime($temp['xaxis']))] = $temp;
			}			
		}
		$lineChartResultSet = array_values($newTempArr);
		$lineChart = array();
