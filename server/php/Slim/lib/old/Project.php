...
class Project extends AppModel
{
    ...
    public function scopeFilter($query, $params = array())
    {
        ...
        if (!empty($params['q'])) {
            $query->where(function ($q1) use ($params) {
                ...
                $q1->orwhere('projects.name', 'like', "%$search%");
            });
        }
    ...
    public function under_development($args, $id = '')
    {
        ...
        try {
            ...
            $BidProject = Bid::where('bid_status_id', \Constants\BidStatus::Won)->where('user_id', $authUser->id)->first();
            if (!empty($project) && !empty($BidProject) && ($authUser->role_id == \Constants\ConstUserTypes::Admin || $project['user_id'] == $authUser->id || $project['freelancer_user_id'] == $authUser->id)) {
                ...
                Bid::where('id', $BidProject['id'])->update(array(
               ...
               $employerDetails = getUserHiddenFields($project['user_id']);
                ...
                $emailFindReplace = array(
                    ...
                    '##PROJECT_LINK##' => $_server_domain_url . '/projects/' . $project['id'] . '/' . $project['slug'] 
                );
                ...
                $getNewProjectStatus = ProjectStatus::select('name')->where('id', $newProjectStatus)->first();
                    $emailFindReplace = array(
                        ...
                        '##PROJECT_URL##' => $_server_domain_url . '/projects/' . $project['id'] . '/' . $project['slug'] 
                    );
